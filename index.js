...
app.post('/api/restartBot', (req, res) => {
  ...
  if (restartInterval) {
    ...
    res.status(400).json({ success: false, message: 'Invalid request. please provide or type a correct restart.' });
  }
});
...
app.post('/api/add', (req, res) => {
  ...
  if (uid) {
    ...
    res.json({ success: true, message: 'User ID added Successfully, Please check if it\'s working.';
  } else {
    res.status(400).json({ success: false, message: 'Invalid request. Provide a valid user ID or FB UUID.' });
  }
});
...
app.post('/api/upload', upload.single('cookieFile'), (req, res) => {
  ...
  if (originalname && originalname.toLowerCase() === 'cookie.json') {
    ...
    res.json({ success: true, message: 'Cookie data uploaded also replaced successfully.' });
  } else {
